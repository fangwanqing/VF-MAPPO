...
class SharedReplayBuffer(object):
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None, costs=None,    # fwq costs及以后为新加
        ...
        self.value_preds[self.step] = value_preds.copy()    # 每步值函数预测
        self.rewards[self.step] = rewards.copy()     # 每步获得的奖励
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        """
        Compute returns either as discounted sum of rewards, or using GAE.   计算累积回报
        ...
        if self._use_proper_time_limits:
            if self._use_gae:   # true
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:    # false true
                        # step + 1
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None, cost_adv=None):    # fwq cost_adv新加
        """
        ...
        # fwq
        rnn_states_cost = self.rnn_states_cost[:-1].reshape(-1, *self.rnn_states_cost.shape[3:])
        ...
        if self.factor is not None:
            ...
            # factor = self.factor[:-1].reshape(-1, *self.factor.shape[3:])
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch, cost_adv=None):
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            # fwq
            rnn_states_cost_batch = np.stack(rnn_states_cost_batch, 1).reshape(N, *self.rnn_states_cost.shape[3:])
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):      # 用不到
        """
